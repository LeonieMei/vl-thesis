...
#' Impute missing variant data where possible.
#'
#' @param pcr_dates A list of the PCR dates of an infection for which the variant 
#' is not known.
#' @param days The day of each PCR relative to the first PCR of that infection.
#'
#' @return A string specifying the variant an infection is assigned to. This is 
#' only done for infections that have no variant assigned yet (because they were 
#' done at transition times from one variant to the next), so the assignment 
#' here will be based on a 50% threshold (>= 50% of samples from a specific 
#' variant).
impute_variant <- function(pcr_dates, days, variants, has_typing, prob = FALSE) {
  # Make sure that we are only dealing with infections that don't have any 
  # variants assigned (and also no typing PCR).
  assert_that(all(is.na(variants)))
  assert_that(all(has_typing == FALSE))
  
  # Variant dominant time thresholds (>= 90% of samples from that variant).
  wt_end <- as.Date("2021-02-07")
  alpha_start <- as.Date("2021-03-29")
  days_wt_alpha <- as.numeric(alpha_start - wt_end)
  
  alpha_end <- as.Date("2021-05-23")
  delta_start <- as.Date("2021-07-12")
  days_alpha_delta <- as.numeric(delta_start - alpha_end)
  
  delta_end <- as.Date("2021-12-19")
  omicron_start <- as.Date("2022-01-17")
  days_delta_omicron <- as.numeric(omicron_start - delta_end)
  
  assert_that(days_wt_alpha == 50)
  assert_that(days_alpha_delta == 50)
  assert_that(days_delta_omicron == 29)
  
  # It can be the case that several PCRs have been performed on the first day. 
  # --> select first entry
  first_pcr_date <- pcr_dates[days == 0][1]
  
  # If no variant was assigned and no typing PCR performed, it must be because
  # it was performed at a time of transition from one variant to the next.
  # Here, we calculate probabilities based on manually created logistic curves
  # that simulate the growth of the emerging variant.
  
  variant <- NA_character_
  
  if ((wt_end < first_pcr_date) & (first_pcr_date < alpha_start)) {
    x <- as.numeric(first_pcr_date - wt_end)
    # Probability of variant being Alpha (as opposed to Wildtype).
    p <- 1 / (1 + exp(1 / 12 * -(x - days_wt_alpha / 2)))
    variant <- if (p <= 0.5) "Wildtype" else "Alpha"
    # Probabilities for wildtype, alpha, delta, omicron
    probs <- c(1 - p, p, 0, 0)
  } else if ((alpha_end < first_pcr_date) & (first_pcr_date < delta_start)) {
    x <- as.numeric(first_pcr_date - alpha_end)
    # Probability of variant being Delta (as opposed to Alpha).
    p <- 1 / (1 + exp(1 / 12 * -(x - days_alpha_delta / 2)))
    variant <- if (p <= 0.5) "Alpha" else "Delta"
    probs <- c(0, 1 - p, p, 0)
  } else if ((delta_end < first_pcr_date) & (first_pcr_date < omicron_start)) {
    x <- as.numeric(first_pcr_date - delta_end)
    # Probability of variant being Omicron (as opposed to Delta).
    p <- 1 / (1 + exp(1 / 7 * -(x - days_delta_omicron / 2)))
    variant <- if (p <= 0.5) "Delta" else "Omicron"
    probs <- c(0, 0, 1 - p, p)
  }
