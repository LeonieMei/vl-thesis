...
for min_tests in {3..7}; do
  for min_pos_tests in {2..7}; do
    ...
    time Rscript post_pred.R --model_stan 2 --threading --min_tests ${min_tests} --min_pos_tests ${min_pos_tests} --n_iter_sampling 1000 --n_chains 4 --simulate_data --filter_by_testname "T2" --trim_neg_pcrs --exclude_lrt_samples --min_max_load 5 --color_by "is.difficult" --make_only_mean_figure
  done
done
