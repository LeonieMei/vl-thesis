// Like Model 2 but with tighter priors. 
// The location parameter of the prior for the clearance rate was updated, too.
...
functions {
  real partial_sum(int[] gstart_DAY_slice,
    ...
    for(g2 in start_idx:end_idx){
      ...
      vector[N_g] DAY_shifted1 = X_DAY2[gstart_DAY_slice[gstart_idx]:gend_DAY[g2]] + shift[g2];
      vector[N_g] yhat1;
...
data {
  ...
  vector[N_DAY] X_DAY2;                              // day of measurement in time series (day 0 is day of highest viral load)
...
parameters {
  ...
  vector<lower=-8,upper=30>[G] shift;
...
model {
  // DAY
  log_intercept_mu ~ normal(2.2,.1); 
  log_slope_up_mu ~ normal(.5,.2);
  log_slope_down_mu ~ normal(-1.2,.3);
  ...
  betaPGH_intercept ~ normal(0,.1);
  betaPGH_slope_up ~ normal(0,.15);
  betaPGH_slope_down ~ normal(0,.15);
  ...
  target += reduce_sum(partial_sum, gstart_DAY, grainsize,
                       gend_DAY, Y_DAY_imputed, X_DAY2, shift, 
